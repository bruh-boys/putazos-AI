...
// reutiliza la memoria, para poder generar una cadena de acciones
func (net NN) Predict(input []float32, memory [][]float32) ([]float32, [][]float32) {
	lays, feed := net.FeedFoward(input, memory)
	return lays[len(lays)-1], feed

}

func (net *NN) BackPropagation(layers [][]float32, memory [][]float32, expected []float32) ([][][]float32, [][]float32) {
